...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--data", help="Dia a ingerir (YYYY-MM-DD). Default: ontem.", default=None)
    parser.add_argument(
        "--familia",
    ...
    parser.add_argument("--skip-reconcile", action="store_true", help="Não rodar reconciliação")
    parser.add_argument("--skip-dims", action="store_true", help="Não sincronizar dimensões (dim_*)")
    parser.add_argument("--only-dims", action="store_true", help="Só sincronizar dimensões (ignora fatos)")
            ...
            with postgres_connect() as pg:
                ...
                for n in nomes:
                    if n not in FAMILIAS:
                        ...
                        return 1
                for nome in nomes:
                    with span(f"ingest_{nome}", familia=nome, data=dia.isoformat()):
                        n = FAMILIAS[nome](mysql, pg, dia)
                        print(f"  {nome}: {n} linhas agregadas")
                if not args.skip_reconcile:
                    with span("reconcile_day", data=dia.isoformat()):
